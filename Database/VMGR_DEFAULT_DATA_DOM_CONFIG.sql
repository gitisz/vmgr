...
IF @@SERVERNAME LIKE 'DEV2\SQL1'	
	BEGIN
		SET @Environment = 'DEV2';
		SET @SqlInstanceName = 'DEV2\SQL1';
	END
...
IF NOT EXISTS (SELECT * FROM vmg.Setting WHERE [Key] = N'DefaultPackageRelationship')
	BEGIN
		IF @Environment = 'LOCAL'
			BEGIN
				PRINT 'ADDING THE [DefaultPackageRelationship] SETTING TO [' + @Environment + '].';
				INSERT [vmg].[Setting] ([Key], [Value], [Cache], [CreateDate], [CreateUser], [UpdateDate], [UpdateUser]) VALUES (N'DefaultPackageRelationship', N'http://schemas.dominionnet.com/package/2013/vmgx', 1, GETDATE(), N'TODD105-SA', NULL, NULL)
			END
		IF @Environment = 'DEV2'
			BEGIN
				PRINT 'ADDING THE [DefaultPackageRelationship] SETTING TO [' + @Environment + '].';
...
IF NOT EXISTS (SELECT * FROM vmg.Setting WHERE [Key] = N'DefaulPort')
	BEGIN
		IF @Environment = 'LOCAL'
			BEGIN
				PRINT 'ADDING THE [DefaulPort] SETTING TO [' + @Environment + '].';
				INSERT [vmg].[Setting] ([Key], [Value], [Cache], [CreateDate], [CreateUser], [UpdateDate], [UpdateUser]) VALUES (N'DefaulPort', N'8000', 1, GETDATE(), N'TODD105-SA', NULL, NULL)
			END
		IF @Environment = 'DEV2'
			BEGIN
				PRINT 'ADDING THE [DefaulPort] SETTING TO [' + @Environment + '].';
